...
- After adding or changing a Mine A Shape! picture in `js/mineshapes.js`: `npm run openings`
  (rewrites `js/mineopenings.js` with the solvable openings and the picture's difficulty
  profile; commit both). The tests fail until it has been run. Add `-- --profile-all` to
  measure difficulty from every opening rather than a sample of five.
...
- `js/minesweeper.js` — Minesweeper rules: mine placement, cascades, flags, chording,
  win/lose. Immutable, so every function returns a new game. A tap on a mine asks
  `js/minesolver.js` whether the guess was forced before it loses.
...
- `js/mineopenings.js` — generated by `npm run openings` (`tools/mine-openings.mjs`), never
  edited by hand: every solvable opening of every picture, with a fingerprint of its rows and
  its difficulty profile. The profile is measured by `tools/mine-difficulty.mjs`, which is
  dev-only and never shipped; nothing about difficulty reaches the player.
