...
import { resolveForcedGuess, findSafeSquare } from './minesolver.js';
...
export function createGame(options = {}) {
    ...
    return {
        ...
        height,
        mineCount,
        shape: null,       // the Mine A Shape! picture when the mines draw one; see createShapeGame
        mines: new Set(),
...
/* Whether a position can be played to the end by reasoning alone: open a square the
   numbers prove safe, again and again, until every safe square is open - never a guess.
   Mines need no move of their own; once every safe square is open, the count settles
   them. */
export function solvesWithoutGuessing(game) {
    let current = game;
    while (current.revealed.size < safeCellCount(current)) {
        const proof = findSafeSquare(current);
        if (proof.result !== 'safe') return false;
        current = reveal(current, proof.x, proof.y);
    }
    return true;
}

// A Mine A Shape! picture's mines, laid on a full rectangle, with nothing opened yet.
function layShape(shape) {
    const board = createGame({ width: shape.width, height: shape.height, mineCount: shape.mineCount });
    return withCounts(copy(board, { mines: new Set(shape.mines), status: STATUS.PLAYING, shape }));
}

// Squares with no mine on or around them: opening one cascades.
function quietSquares(game) {
    const out = [];
    for (let y = 1; y <= game.height; y++) {
        for (let x = 1; x <= game.width; x++) if (!isMine(game, x, y) && countAt(game, x, y) === 0) out.push({ x, y });
    }
    return out;
}
...
/* Every opening a picture offers - what tests/mineshapes checks each picture against. None
   means the picture cannot be played without guessing, and has to be reworked. */
export function solvableStarts(shape) {
...
export function reveal(game, x, y, random = Math.random, { forgive = true, now, budgetMs } = {}) {
    ...
    if (isMine(started, x, y)) {
        /* Never on a Mine A Shape! board: its mines are the picture, and moving them would
           break it. A forced guess there loses, as in classic Minesweeper - the project
           owner's call. */
        const mines = forgive && game.status === STATUS.PLAYING && !game.shape
